...
class AuthorityRole:
    ...
    async def __call__(self, request: Request):
        ...
        roles = await e.get_roles_for_user(request.state.user.username)
        if not roles or self.role not in roles:
            raise AuthenticationError(err_desc=f'Role denied: [{self.role}]')
