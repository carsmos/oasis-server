...
class DynamicSceneRepoImpl(DynamicScenesRepo):
    ...
    async def create_scenario(self, scenario: DynamicScenesAggregate):
    ...
    async def delete_scenario_by_id(self, dynamic_scene_id: str):
        filter = {"id": dynamic_scene_id}
        filter.update({"usr_id": self.user.id})
        await self.scenarios_collection.delete_one(filter)

    async def update_scenario(self, dynamic_scene_id: str, update_scenario: DynamicScenesAggregate):
        ...
        filter = {'id': dynamic_scene_id}
        filter.update({"usr_id": self.user.id})
        await self.scenarios_collection.update_one(filter,
                                             {'$set': update_scenario_DO.dict(exclude={'create_time','usr_id'})})

    async def get(self, dynamic_scene_id: str):
        filter = {'id': dynamic_scene_id}
        filter.update({"usr_id": self.user.id})
