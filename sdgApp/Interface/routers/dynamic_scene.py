...
async def create_dynamic_scene(dynamic_scene_create_model: ScenarioCreateDTO, db=Depends(get_db)):
    try:
        return ScenarioCommandUsercase(db_session=db).create_scenario(dynamic_scene_create_model)
    except:
        raise
...
async def delete_dynamic_scene(dynamic_scene_id: str, db=Depends(get_db)):
    try:
        return ScenarioDeleteUsercase(db_session=db).delete_scenario(dynamic_scene_id)
    except:
        raise
...
async def update_dynamic_scene(dynamic_scene_id: str,
                               ...
                               db=Depends(get_db)):
    try:
        result = ScenarioUpdateUsercase(db_session=db).update_scenario(dynamic_scene_id,
